...
begin
  ...
  -- fornecimento: stock disponível reserva sempre; só a falta gera necessidade de compra/produção
  for it in
  ...
  loop
    ...
    select vendavel into v_disp from erp.stock_atual where produto_id = it.produto_id;

    if it.forn = 'stock' or coalesce(v_disp, 0) >= it.quantidade then
