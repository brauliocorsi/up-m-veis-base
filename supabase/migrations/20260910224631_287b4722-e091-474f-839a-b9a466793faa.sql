...
begin
  ...
  for it in
    ...
    order by i.linha
  loop
    insert into erp.stock_atual (produto_id) values (it.produto_id)
    on conflict (produto_id) do nothing;
    select greatest(vendavel, 0) into v_disp
      from erp.stock_atual where produto_id = it.produto_id for update;
    ...
    elsif it.forn in ('compra', 'producao') then
      v_reservar := least(coalesce(v_disp, 0), it.quantidade);
      v_falta := it.quantidade - v_reservar;
      v_reserva := null;

      if v_reservar > 0 then
        v_reserva := erp.reservar(it.produto_id, v_reservar, 'pedido', p_pedido_id, it.id, null);
      end if;
...
begin
  if exists (
    select 1 from erp.pedido_itens
    where id = 'f7dbc5df-bdf6-4b6b-8c50-4e5a04af4050'
      and pedido_id = 'b7fe5b69-6f92-4b88-a28e-522a0b9793db'
      and reserva_id is null
  ) then
    perform set_config('erp.motor', '1', true);
    insert into erp.reservas (
      produto_id, quantidade, documento_tipo, documento_id, linha_id, estado
    ) values (
      'da544ca6-3998-4131-b86d-d8ddaeb2d0d1', 2, 'pedido',
      'b7fe5b69-6f92-4b88-a28e-522a0b9793db',
      'f7dbc5df-bdf6-4b6b-8c50-4e5a04af4050', 'ativa'
    ) returning id into v_reserva;

    update erp.pedido_itens
       set reserva_id = v_reserva,
           estado = 'encomendado'
     where id = 'f7dbc5df-bdf6-4b6b-8c50-4e5a04af4050';

    update erp.necessidades_compra
       set quantidade = 1
     where pedido_id = 'b7fe5b69-6f92-4b88-a28e-522a0b9793db'
       and item_id = 'f7dbc5df-bdf6-4b6b-8c50-4e5a04af4050'
       and estado in ('aberta', 'encomendada');
    perform set_config('erp.motor', '', true);
  end if;
end $repair$;