...
begin
  ...
  if TG_OP = 'UPDATE'
     and (NEW.nota_pdf_path is distinct from OLD.nota_pdf_path
          or NEW.nota_pdf_em is distinct from OLD.nota_pdf_em)
     and NEW.* is not distinct from (OLD.*
           #= hstore('nota_pdf_path', NEW.nota_pdf_path) || hstore('nota_pdf_em', NEW.nota_pdf_em::text)
         ) then
    return NEW;
  end if;
